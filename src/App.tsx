...
export default function App() {
  ...
  // If viewUserId is set (for modal profiles) OR activeFriendTheme is set (for native Friends tab browsing)
  const isViewingFriendProfile = (activeTab === "friends" || activeTab === "terminal") && (viewUserId !== null || activeFriendTheme !== null);
  ...
  const handleTagDeleted = (kind: TagKind, tag: string) => {
    if (kind === "confluence") setConfluenceLibrary((prev) => prev.filter((item) => item !== tag));
    else if (kind === "mistake") setMistakeLibrary((prev) => prev.filter((item) => item !== tag));
    else setEntryLibrary((prev) => prev.filter((item) => item !== tag));

    const updatedTradesList: TradeRecord[] = [];
    setTrades((previous) => {
      let changed = false;
      const res = previous.map(t => {
        let newTags;
        if (kind === 'confluence' && t.confluenceTags?.includes(tag)) {
          newTags = { confluenceTags: t.confluenceTags.filter(x => x !== tag) };
        } else if (kind === 'mistake' && t.mistakeTags?.includes(tag)) {
          newTags = { mistakeTags: t.mistakeTags.filter(x => x !== tag) };
        } else if (kind === 'entry' && t.entryTags?.includes(tag)) {
          newTags = { entryTags: t.entryTags.filter(x => x !== tag) };
        }
        if (newTags) {
          changed = true;
          const up = { ...t, ...newTags };
          updatedTradesList.push(up);
          return up;
        }
        return t;
      });
      return changed ? res : previous;
    });

    if (user && !isDemoMode) {
      setTimeout(async () => {
        for (const t of updatedTradesList) {
          if (t.id) {
            await supabase.from('trades_v2').update({
              confluence_tags: t.confluenceTags,
              mistake_tags: t.mistakeTags,
              entry_tags: t.entryTags,
            }).eq('id', t.id);
  ...
  const handleAccountsDeleted = (accountIds: string[]) => {
    const updatedTradesList: TradeRecord[] = [];
    setTrades((previous) => {
      let changed = false;
      const res = previous.map(t => {
        if (t.accountId && accountIds.includes(t.accountId)) {
          changed = true;
          const up = { ...t, accountId: undefined };
          updatedTradesList.push(up);
          return up;
        }
        return t;
    ...
    if (user && !isDemoMode) {
      setTimeout(async () => {
        for (const t of updatedTradesList) {
          if (t.id) {
            await supabase.from('trades_v2').update({ account_id: null }).eq('id', t.id);
          }
        }
      }, 100);
    }
      ...
      <main className="relative z-10 mx-auto max-w-[92rem] px-4 py-6 sm:px-6 sm:py-8">
        ...
        {activeTab === "accounts" && (
          <AccountsLab 
            ...
            onAccountsDeleted={handleAccountsDeleted}
            panelStyle={panelStyle}
            softPanelStyle={softPanelStyle}
            panelTint={panelTint}
                    ...
                    <Reorder.Group axis="y" values={entryLibrary} onReorder={setEntryLibrary} className="mt-3 grid grid-cols-2 gap-2">
                      {entryLibrary.map((tag) => (
                        <Reorder.Item key={tag} value={tag} className="tag-chip flex justify-between items-center bg-white/5 pl-3 pr-1 py-1 text-sm text-slate-200 cursor-grab active:cursor-grabbing rounded-lg border border-white/10" drag transition={{ duration: 0 }}>
                          ...
                          <button onClick={() => handleTagDeleted("entry", tag)} className="shrink-0 p-1.5 rounded-md hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 transition-colors">
                            <X className="h-3 w-3" />
                          </button>
                        </Reorder.Item>
                      ))}
                  ...
                  <div>
                    ...
                    <Reorder.Group axis="y" values={confluenceLibrary} onReorder={setConfluenceLibrary} className="mt-3 grid grid-cols-2 gap-2">
                      {confluenceLibrary.map((tag) => (
                        <Reorder.Item key={tag} value={tag} className="tag-chip flex justify-between items-center bg-white/5 pl-3 pr-1 py-1 text-sm text-slate-200 cursor-grab active:cursor-grabbing rounded-lg border border-white/10" drag transition={{ duration: 0 }}>
                          <span className="pointer-events-none truncate mr-1">{tag}</span>
                          <button onClick={() => handleTagDeleted("confluence", tag)} className="shrink-0 p-1.5 rounded-md hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 transition-colors">
                            <X className="h-3 w-3" />
                          </button>
                        </Reorder.Item>
                      ))}
                  ...
                  <div>
                    ...
                    <Reorder.Group axis="y" values={mistakeLibrary} onReorder={setMistakeLibrary} className="mt-3 grid grid-cols-2 gap-2">
                      {mistakeLibrary.map((tag) => (
                        <Reorder.Item key={tag} value={tag} className="tag-chip flex justify-between items-center bg-white/5 pl-3 pr-1 py-1 text-sm text-slate-200 cursor-grab active:cursor-grabbing rounded-lg border border-white/10" drag transition={{ duration: 0 }}>
                          <span className="pointer-events-none truncate mr-1">{tag}</span>
                          <button onClick={() => handleTagDeleted("mistake", tag)} className="shrink-0 p-1.5 rounded-md hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 transition-colors">
                            <X className="h-3 w-3" />
                          </button>
                        </Reorder.Item>
                      ))}
